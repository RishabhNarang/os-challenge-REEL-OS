...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    while (1) {
        ...
        /*A--- The hash doesn't have to be swapped in byte order since the hashing done by openssl will return in the same
	...
	if(mh.heapSize == 500) { 
		...
		while(mh.heapSize > 0) { 
			...
			// Switch from little endian for network
			result = htobe64(result);

			//Send message to client
			send(maxRN.clientfd, &result, sizeof(result), 0);	
